    ...
    <el-tabs tab-position="left" :style='{"border":"2px solid #a4d9b7","padding":"20px","margin":"20px 0 0 0","borderRadius":"8px","background":"rgba(255,255,255,1)"}' @tab-click="handleClick">
      <el-tab-pane label="个人中心">
        <el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="80px">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='yonghu'" label="家庭住址" prop="jiatingzhuzhi">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiajiaolaoshi'" label="教师工号" prop="jiaoshigonghao">
            <el-input v-model="sessionForm.jiaoshigonghao" placeholder="教师工号" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiajiaolaoshi'" label="密码" prop="mima">
            <el-input type="password" v-model="sessionForm.mima" placeholder="密码"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiajiaolaoshi'" label="教师姓名" prop="jiaoshixingming">
            <el-input v-model="sessionForm.jiaoshixingming" placeholder="教师姓名" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiajiaolaoshi'" label="头像">
            <el-upload
              class="avatar-uploader"
              :action="uploadUrl"
              :headers="headers"
              :show-file-list="false"
              :on-success="jiajiaolaoshitouxiangHandleAvatarSuccess">
              <img :style='{"border":"1px solid #9ce0b5","cursor":"pointer","minHeight":"100px","boxShadow":"inset 0px 0px 48px 0px #cef1db","color":"#3db769","borderRadius":"6px","textAlign":"center","width":"150px","fontSize":"32px","lineHeight":"100px","height":"auto"}' v-if="sessionForm.touxiang" :src="baseUrl + sessionForm.touxiang" class="avatar">
              <i v-else class="el-icon-plus avatar-uploader-icon"></i>
            </el-upload>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiajiaolaoshi'" label="性别">
            <el-select v-model="sessionForm.xingbie" placeholder="请选择性别" >
              <el-option v-for="(item, index) in dynamicProp.xingbie" :key="index" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiajiaolaoshi'" label="学历">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiajiaolaoshi'" label="专业" prop="zhuanye">
            <el-input v-model="sessionForm.zhuanye" placeholder="专业" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiajiaolaoshi'" label="教师邮箱" prop="jiaoshiyouxiang">
            <el-input v-model="sessionForm.jiaoshiyouxiang" placeholder="教师邮箱" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiajiaolaoshi'" label="联系电话" prop="lianxidianhua">
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      if ('jiajiaolaoshi' == this.userTableName) {
      ...
      if ('jiajiaolaoshi' == this.userTableName) {
        this.$set(this.sessionForm, 'jiaoshiyouxiang', null);
      }
      if ('jiajiaolaoshi' == this.userTableName) {
      ...
      if ('jiajiaolaoshi' == this.userTableName) {
        this.$set(this.rules, 'xueli', [{ required: true, message: '请输入学历', trigger: 'blur' }]);
			...
			if ('jiajiaolaoshi' == this.userTableName) {
        this.$set(this.rules, 'jiaoshiyouxiang', [{ required: false, validator: this.$validate.isEmail, trigger: 'blur' }]);
      }
			if ('jiajiaolaoshi' == this.userTableName) {
    ...
    methods: {
      init() {
        ...
        if ('jiajiaolaoshi' == this.userTableName) {
          this.dynamicProp.xueli = '专科,本科,研究生'.split(',');
        }
      },
      getSession() {
      ...
      jiajiaolaoshitouxiangHandleAvatarSuccess(res, file) {
        if (res.code == 0) {
          if ('jiajiaolaoshi' == this.userTableName) {
            this.sessionForm.touxiang = 'upload/' + res.file;
          }
